...
について、Softmax型の関数につっこんで確率を作ったものでした。これらの分子と分母をそれぞれ$\exp(y)$で割ってみましょう。
...
です。パラメータをSoftmax関数を使って確率を作る場合、パラメータは差しか関係がないのですから、両者の事象のパラメータに含まれる$b_c$は無視してOKです。以上から、
...
$$
P(v_a=0, h_c=1) = \frac{1}{\exp(b_a + W_{ac}) + 1}
$$
...
カレーフェアがあった日は56+8=64日です。そのうち、Aliceが学食に来たのは8日、つまり1/8の確率でしか学食に来ません。
...
ただし和は$v_a = 0, 1$、$h_c = 0,1$の4通りについて取ります。右辺は、カレーフェアの有無とAliceが学食にくる確率との関係を表すモデルであるとみなすことができます。$P(v_a, h_c)$の変数$v_a$、$h_c$に0と1の好きな値を代入すれば、求めたい確率を得ることができます。モデルパラメータは、バイアス$b_a$と重み$W_{ac}$です。パラメータとして、カレーフェアのバイアス$b_c$も入っていますが、これは現時点では分子分母でキャンセルしてしまうので、値が決まりません。
...
ここで、$b_1 = b_a, b_2 = b_b, b_3 = b_c, J_{12} = 0, J_{13} = J_{ac},W_{23} = J_{bc}$です。
...
このように、状態を表す変数の値が0か1の二値であり、ある変数の状態の実現確率がボルツマン重みで記述されるようなモデルをボルツマンマシン(Bolzmann machine)と呼びます。ボルツマンマシンは、パラメータが決まってしまえば、任意の変数の状態の同時確率や条件付き確率を計算することができます。
