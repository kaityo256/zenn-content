...
ただし和は$v_a = 0, 1$、$h_c = 0,1$の4通りについて取ります。
...
Aliceの場合と同様に、カレーフェアが無い日にBobが学食に来る確率から、Bobのバイアス$b_b$が求まります。ちゃんと書くと、
...
ここまでで、AliceとBobのバイアス$b_a, b_b$と、カレーフェアとの重み$W_{ac}, W_{bc}$が求まりました。残りはカレーフェアのバイアス$b_c$だけです。こちらを求めるのは簡単で、AliceとBobが両方学食に来ていない時のカレーフェアの開催確率を見ればOKです。最初の表から関連するところを抜粋してみましょう。
